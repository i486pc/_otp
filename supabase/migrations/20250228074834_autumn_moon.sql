/*
  # Create OTP Authentication Tables

  1. New Tables
    - `_otp` - Stores user information
      - `id` (uuid, primary key)
      - `name` (text)
      - `phone` (text)
      - `email` (text)
      - `authentication_by` (text)
    ...
    - `_otp_verification` - Stores verification status
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key)
...
CREATE TABLE IF NOT EXISTS _otp (
  ...
  phone TEXT,
  email TEXT,
  authentication_by TEXT CHECK (authentication_by IN ('sms', 'email', 'call', 'whatsapp', 'totp')),
...
CREATE TABLE IF NOT EXISTS _otp_codes (
  ...
  code TEXT NOT NULL,
  channel TEXT CHECK (channel IN ('sms', 'email', 'call', 'whatsapp', 'totp')),
